...
class CanvasWrapper:
    def __init__(self, root, size, offset = (0,0), bg = "white"):
        self.offset = offset
        self.size = size
    ...
    def placeOver(self, image:numpy.ndarray, position:list|tuple, center = False):
        if center:
            ...
            copy = getRegion(image, (self.offset[0]-position[0],self.offset[1]-position[1]), (self.offset[0]+self.size[0]-position[0], self.offset[1]+self.size[1]-position[1]))
